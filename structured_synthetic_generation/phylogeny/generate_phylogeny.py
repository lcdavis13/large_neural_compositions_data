...
def generate_tree(feature_num, feature_depth, num_leaves, tree_depth, stddev_decay_rate, parent_features=None, parent_rate=0.0, stddev=1.0):
    if parent_features is None:
        ...
        intrinsic_rate = np.random.normal(loc=parent_rate, scale=stddev)
    ...
    if tree_depth > 1:
        ...
        for leaves_in_child in child_leaves:
            child = generate_tree(
                feature_num, feature_depth, leaves_in_child, tree_depth - 1, stddev_decay_rate,
                parent_features=features, parent_rate=intrinsic_rate, stddev=stddev * stddev_decay_rate
            )
...
p.feature_depth = 6        # Depth of each vector (N)
...
p.stddev_decay_rate = 0.5   # Decay rate for standard deviation

tree = generate_tree(p.feature_num, p.feature_depth, p.num_leaves, p.tree_depth, p.stddev_decay_rate)
